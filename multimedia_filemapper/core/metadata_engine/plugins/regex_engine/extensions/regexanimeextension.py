...
class RegexAnimeExtension:
    ...
    def get_name(self, stream, season_directory=False):
        ...
        :return: NAME
        """
        _uploader_patterns = '\[HorribleSubs\]|\[AnimeRG\]|' \
                             '\[Krosis\]|' \
                             '\[Dcms-Fansubs\]|' \
                             '\[Ohys-Raws\]|' \
                             '\[PuyaSubs!\]|FUM|DIMENSION|PODO|ROVERS'
        ...
        try:
            ...
            try:
                ...
                name = string_builder.prettify_stream(tail[header:-core])
            ...
            name = tail[header:-2]
            # print(f'{self.name}: {stream} :: name:{name}')
            return string_builder.prettify_stream(name)

    def get_episode(self, stream):
        """
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    episode = episode[1:]
                    # print(f'{self.name}: {stream} :: episode:{episode}')
                    return episode
            else:
                episode = episode[8:]
                # print(f'{self.name}: {stream} :: episode:{episode}')
            ...
            episode = episode[2:]
            # print('{self.name}: {stream} :: episode:{episode}')
            return str(episode)
