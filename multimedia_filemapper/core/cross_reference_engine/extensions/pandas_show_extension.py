...
from multimedia_filemapper.core.metadata_engine.plugins.tvdb_engine.extensions.tvdbshowextension import TVDbShowExtension
...
class PandasShowExtension:
    ...
    def create_default_show_tree_directory(self, dataframe, current_show, debug=False):
        ...
        for episode_index in dataframe_episodes.index.tolist():
            ...
            # We sieve_engine if all the season directories are created if not
            ...
            # Now we sieve_engine if every episode it's in his own directory
            ...
            for i in range(0, total_seasons, 1):
                try:
                    ...
                    if not dataframe_season.empty:
                        season_dir = 'YES'
                    print(f'Directory: {season_dir} | Season {i} - Found ({current_episodes}/{total_episodes} Episodes)')
                    ...
                    for index in range(0, len(dataframe_temp.index), 1):
                        ...
                        if not dataframe_directory.empty:
                            current_dir = 'YES'
                        print(f'--: Directory: {current_dir} | {basename}')
                print('------' * 20)
