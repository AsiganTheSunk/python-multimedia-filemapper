...
from main.datastructure.Metadata import Metadata
from main.datastructure.TreeRoot import TreeRoot
from main.retrieve import online_retrieve_module as onrmod
...
def create_default_anime_tree_directory(dataframe, current_anime=str, library=str, with_dir=True):
    ...
    for index in range(0, len(dataframe_temp.index), 1):
        ...
        if parent not in 'Animes':
            real_index = real_index.tolist()[0]
            dataframe = update_parent_dataframe_row(dataframe=dataframe, index=real_index, parent=current_anime)
    return dataframe
...
def update_tree_info(old_dataframe, dataframe, tree):
    ...
    print('--: Nodes that need to be Updated in the tree: ('+ str(len(old_dataframe)) + '/' + str(total_rows) + ')')
