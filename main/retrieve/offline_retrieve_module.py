from main.datastructure.FileFlags import FileFlags as FFLAGS
import re
...
def retrieve_offline_audio(path=None, verbose=None):
    ...
    '''
    try:
        audio = re.search('((AC3)|(DTS)|(DD5\.1)|(ACC(2\.0)?)|(MP3))', path, re.IGNORECASE).group(0)
    except Exception as e:
        audio = ''
        return audio
    else:
...
def retrieve_offline_codec(path=None, verbose=None):
    ...
    '''
    try:
        codec = re.search('((x264)|(H264)|(x265)|(H265)|(XviD)|(DivX))', path, re.IGNORECASE).group(0)
    except Exception as e:
        codec = ''
        return codec
    else:
...
def retrieve_extension(path=None, verbose=None):
    ...
    '''
    try:
        extension = re.search('(\.mkv|\.mp4|\.srt|\.sub|\.ass)', path).group(0)
    except Exception as e:
        extension = ''
        return extension
    else:
...
def retrieve_source(path=None, verbose=None):
    ...
    for item in source_list:
        try:
            source = re.search('((\[)?' + re.escape(item) + '(\])?)', path, re.IGNORECASE).group(0)
        except Exception as e:
            source = ''
            return source
        else:
...
def retrieve_uploader(path=None, verbose=None):
    ...
    for item in uploader_list:
        try:
            uploader = re.search('((\[)?'+re.escape(item)+'(\])?)',path, re.IGNORECASE).group(0)
        except Exception as e:
            uploader = ''
            return uploader
        else:
