...
class PandasAnimeExtension():
    ...
    def create_default_library(self, dataframe, root_basename):
        ...
        dataframe = self.create_animes_directory(dataframe=dataframe,
                                                 root_basename=root_basename)
        for current_anime in unique_anime:
            dataframe = self.create_default_anime_tree_directory(
    ...
    def get_anime_names(self, dataframe):
        ...
        '''
        unique_anime = dataframe.name[
            dataframe['fflag'] == FFLAGS.ANIME_FLAG].unique()
        return unique_anime
    ...
    def get_anime_directories(self, dataframe, current_anime, drop_dup=False):
        ...
        '''
        if drop_dup:
            dataframe_episodes_directories = dataframe.groupby(
                ['name']).get_group(current_anime).drop_duplicates()
        else:
            dataframe_episodes_directories = dataframe.groupby(
                ['name']).get_group(current_anime)
        return dataframe_episodes_directories[
            dataframe_episodes_directories.fflag == FFLAGS.ANIME_DIRECTORY_FLAG]
    ...
    def create_default_anime_tree_directory(self, dataframe, current_anime,
        ...
        main_show_directory = dataframe[dataframe['basename'] == current_anime]
        if main_show_directory.empty:
            dataframe = self.create_main_anime_directory(dataframe=dataframe,
        ...
        dataframe_episodes = self.get_anime(dataframe=dataframe,
                                            current_anime=current_anime)
