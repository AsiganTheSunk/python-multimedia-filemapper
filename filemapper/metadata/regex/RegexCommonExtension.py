...
class RegexCommonExtension():
    ...
    def get_uploader(self, stream, debug=False):
        ...
        :return: UPLOADER
        '''
        _uploader_patterns = ['HorribleSubs|AnimeRG|Krosis|Dmcs-Fansubs|Ohys-Raws|PuyaSubs!|FUM|DIMENSION|PODO|ROVERS']

        try:
            uploader = re.search(_uploader_patterns[0], stream, re.IGNORECASE).group(0)
            ...
            if debug:
                print('{extension_engine}: {stream} :: uploader:{value}').format(extension_engine=self.name,
                                                                        stream=stream,
                                                                        value=uploader)
            return uploader
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    quality = quality[5:] + 'p'
                    if debug:
                        print('{extension_engine}: {stream} :: quality:{value}').format(extension_engine=self.name,
                                                                                stream=stream,
                                                                                value=quality)
                    return quality
            else:
    ...
    def get_acodec(self, stream, debug=False):
        ...
        try:
            ...
            if debug:
                print('{extension_engine}: {stream} :: acodec:{value}').format(extension_engine=self.name,
                                                                        stream=stream,
                                                                        value=acodec)
            return acodec
    ...
    def get_vcodec(self, stream, debug=False):
        ...
        try:
            ...
            if debug:
                print('{extension_engine}: {stream} :: vcodec:{value}').format(extension_engine=self.name,
                                                                        stream=stream,
                                                                        value=vcodec)
            return vcodec
    ...
    def get_source(self, stream, debug=False):
        ...
        try:
            ...
            if debug:
                print('{extension_engine}: {stream} :: source:{value}').format(extension_engine=self.name,
                                                                        stream=stream,
                                                                        value=source)
            return source
    ...
    def get_unwanted(self, stream, debug=False):
        ...
        try:
            ...
            unwanted = stream
            if debug:
                print('{extension_engine}: {stream} :: unwanted:{value}').format(extension_engine=self.name,
                                                                        stream=stream,
                                                                        value=unwanted)
            return unwanted
