...
def validate_show_season_directory(path=None, verbose=None):
    try:
        season_directory = re.search('((\(|\[)?(season(.*)?(\d{1,2}))(\)|\])?)',
                                     path, re.IGNORECASE).group(0)
...
def validate_show_name(path=None, verbose=None):
    try:
        valid = re.search(
            '(\w+\s)*([S]?\d{0,2})([E]\d{2,3})(\-\s(\w+\s)*(\-(\w+\s)*\[\d{3,4}[p]\]))',
            path)
        if valid:
            return True
    except Exception as e:
        ...
        if (fflag in FFLAGS.SHOW_DIRECTORY_FLAG):
            valid = re.search(
                ...
                path)
                    ...
                    fflag in FFLAGS.SHOW_FLAG or FFLAGS.IGNORE_FLAG or FFLAGS.IGNORE_FLAG):
            valid = re.search(
                '(\w+\s)*([S]?\d{0,2})([E]\d{2,3})(\-\s(\w+\s)*(\-(\w+\s)*\[\d{3,4}[p]\]))(\.mkv|\.mp4)',
                path)
