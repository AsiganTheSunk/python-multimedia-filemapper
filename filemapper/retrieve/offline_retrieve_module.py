...
def retrieve_quality(path=None, verbose=None):
    ...
    '''
    try:
        quality = regex.search('(\d{3,4}p)', path).group(0)
    except Exception as e:
        try:
            quality = regex.search('BRRip|HDRip|BluRay|DvdRip|WEB(\-)?DL|WEB(\-)?Rip|HDtv', path, regex.IGNORECASE).group(0)
        except Exception as e:
            try:
                quality = regex.search('\d{4}x\d{3,4}', path, regex.IGNORECASE).group(0)
            except Exception as e:
                quality = ''
                return quality
            else:
...
def retrieve_episode(path=None, verbose=None):
    ...
    '''
    try:
        episode = regex.search ('([e])\d{2,3}', path, regex.IGNORECASE).group(0)
    except Exception as e:
        episode = ''
        return episode
    else:
...
def retrieve_season_directory(path=None, verbose=None):
    ...
    '''
    try:
        season_directory = regex.search('(\(|\[)?s(eason)?(\-|\s|\.)?(\d{1,2})(\)|\])?', path, regex.IGNORECASE).group(0)
        season = regex.search('\d{1,2}', season_directory, regex.IGNORECASE).group(0)
    except Exception as e:
        season = ''
        return season
    else:
...
# TODO Revisar en algun puntos
# TODO Anadir Temp, Temporada y T como elementos reconocidos
def retrieve_season(path=None, verbose=None):
    ...
    '''
    try:
        season = regex.search('([s]\d{2})', path, regex.IGNORECASE).group(0)
    except Exception as e:
        season = ''
        return season
    else:
...
def retrieve_offline_audio(path=None, verbose=None):
    '''
    ...
    '''
    try:
        audio = regex.search('((AC3)|(DTS)|(DD5\.1)|(ACC(2\.0)?)|(MP3))', path, regex.IGNORECASE).group(0)
    except Exception as e:
        audio = ''
        return audio
    else:
...
def retrieve_offline_codec(path=None, verbose=None):
    '''
    ...
    '''
    try:
        codec = regex.search('((x264)|(H264)|(x265)|(H265)|(XviD)|(DivX))', path, regex.IGNORECASE).group(0)
    except Exception as e:
        codec = ''
        return codec
    else:
...
def retrieve_extension(path=None, verbose=None):
    ...
    '''
    try:
        extension = regex.search('(\.mkv|\.mp4|\.srt|\.sub|\.ass)', path).group(0)
    except Exception as e:
        extension = ''
        return extension
    else:
...
def retrieve_source(path=None, verbose=None):
    ...
    for item in source_list:
        try:
            source = regex.search('((\[)?' + regex.escape(item) + '(\])?)', path, regex.IGNORECASE).group(0)
        except Exception as e:
            source = ''
            return source
        else:
...
def retrieve_uploader(path=None, verbose=None):
    ...
    for item in uploader_list:
        try:
            uploader = regex.search('((\[)?' + regex.escape(item) + '(\])?)', path, regex.IGNORECASE).group(0)
        except Exception as e:
            uploader = ''
            return uploader
        else:
...
def retrieve_unwanted(path=None, verbose=None):
    try:
        unwanted = regex.search('(((\.txt)|(\.nfo))$)', path).group(0)
    except Exception as e:
        unwanted = ''
        return unwanted
    else:
...
def retrieve_subtitles_directory(path=None, verbose=None):
    ...
    '''
    try:
        subtitle_directory = regex.search('sub\w{0,6}', path, regex.IGNORECASE).group(0)
    except Exception as e:
        subtitle_directory = ''
...
def retrieve_film_flags(path=None, verbose=None):
    ...
    '''
    try:
        film_flag = regex.search('(EXTENDED(.*)?CUT)|REMASTERED', path, regex.IGNORECASE).group(0)
    except Exception as e:
        film_flag = ''
        return film_flag
    else:
...
def retrieve_film_name(path=None, verbose=None):
    ...
    '''
    try:
        aux_lenth = regex.search('(([1-2])([890])(\d{2}))(?!p)', path).group(0)
        film_name = regex.search('(.*)(([1-2])([890])(\d{2}))(?!p)', path).group(0)[:-(len(aux_lenth))]
    except Exception as e:
        film_name = ''
        return film_name
    else:
...
def retrieve_film_year(path=None, verbose=None):
    ...
    '''
    try:
        film_year = regex.search('(([1-2])([890])(\d{2}))(?!p)', path).group(0)
    except Exception as e:
        film_year = ''
        return film_year
    else:
...
def retrieve_show_name(path=None, verbose=None, fflag=None):
    ...
    try:
        if int(fflag) == int(FFLAGS.SEASON_DIRECTORY_FLAG):
            aux_lenth = regex.search('(\(|\[)?s(eason)?(\-|\s|\.)?(\d{1,2})(\)|\])?', path, regex.IGNORECASE).group(0)
            show_name = regex.search('(.*)(\(|\[)?s(eason)?(\-|\s|\.)?(\d{1,2})(\)|\])?', path, regex.IGNORECASE).group(0)[:-len(aux_lenth)]
        else:
            aux_lenth = regex.search('([s]\d{1,2})', path, regex.IGNORECASE).group(0)
            show_name = regex.search('(.*)([s]\d{1,2})', path, regex.IGNORECASE).group(0)[:-len(aux_lenth)]
    except Exception as e:
        show_name = ''
        return show_name
    else:
...
def retrieve_anime_name(path=None, verbose=None):
    ...
    '''
    try:
        header = len(regex.search('\[(HorribleSubs|Krosis|Dcms-Fansubs|Ohys-Raws|PuyaSubs!)\]', path, regex.IGNORECASE).group(0)) + 1
        tail = regex.search('\[(\w+.*?)\s(\-|x)', path, regex.IGNORECASE).group(0)
    except Exception as e:
        try:
            tail = regex.search('\[(\w+.*?)E(pisode)?(x|\-|\.|\s)?(\d{2,3})', path, regex.IGNORECASE).group(0)
            core = len(regex.search('E(pisode)(\-|\.|\s)?(\d{2,3})', path, regex.IGNORECASE).group(0))
        except Exception as e:
            name = ''
            return name
        else:
...
def retrieve_anime_episode(path=None, verbose=None):
    ...
    '''
    try:
        episode = regex.search('\-.?\d{1,3}', path, regex.IGNORECASE).group(0)
    except Exception as e:
        try:
            episode = regex.search('Episode(\-|\s|\.)?(\d{1,3})', path, regex.IGNORECASE).group(0)
        except Exception as e:
            try:
                episode = regex.search('(x|E)(\d{1,3})', path, regex.IGNORECASE).group(0)
            except Exception as e:
                episode = ''
                return episode
            else:
