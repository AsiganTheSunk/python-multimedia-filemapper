...
def retrieve_subtitles(path=None, verbose=None):
    try:
        subtitles = regex.search('(\.str|\.sub)$', path).group(0)
    except Exception as e:
        subtitles = ''
        return subtitles
    else:
...
def retrieve_str_language(path=None, verbose=False):
    '''
    This function retrieves language from a given path using regex | langdetect
    :param path:
    :param verbose:
    :return:
    '''
    ...
    aux = ''
    try:
        lang = regex.search('\((en|es|spanish|english)\)', path, regex.IGNORECASE).group(0)
    except:
        try:
