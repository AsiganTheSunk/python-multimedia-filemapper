from filemapper.retrieve.imdb import IMDbEngine as imdb
from filemapper.retrieve.regex import RegexEngine as regex
from filemapper.retrieve.subs import SubtitleEngine as subs
from filemapper.retrieve.tvdb import TVDbEngine as tvdb
...
class MetadataEngine():
    def __init__(self):
        self.online_engines = [imdb.IMDbEngine(), tvdb.TVDbEngine(), subs.SubtitleEngine()] #ffprobee.FFProbeExtension()
        self.offline_engines =  [regex.RegexEngine()]
        return


    def map(self, stream, fflag, debug=False):

        # Inicialize Metadata Object to later fill it with the extracted data
        metadata = Metadata()

        # Inicialize container variables for the data to fill
        name=season=episode=ename=film_tag=year=subs=acodec=vcodec=uploader=source=language= ''

        try:
            # First we inicialize the offline functions to parse from the basename of the file or directory
            # Inicialize RegexEngine
